...
# HEAPIFY DOWN
    def heapify_down(self,index) :
        while self.has_left(index) :
            ...
            if self.heap[left_index] < self.heap[smallest] : smallest = left_index

            if self.has_right(index) : 
                right_index = self.right_child(index)
                if self.heap[right_index] < self.heap[smallest] : smallest = right_index

            if index == smallest :           # duplicates are allowed in heap
                break
            else :
...
if __name__ == "__main__" :
    ...
    print("\nExtracted element : ", h.extract_min())
    h.display_list()
    h.display_tree()
    print("\nSorted Heap : ", h.heap_sort())
